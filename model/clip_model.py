...
from .mmencoder_withlora import LoRAMultiheadAttention
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor,modality=None):
...
def convert_weights(model: nn.Module):
    ...
    def _convert_weights_to_fp16(l):
        ...
        if isinstance(l,LoRAMultiheadAttention):
        ...
        for name in ["text_projection", "proj", "mcq_proj","query_projection","projection"]:
            if hasattr(l, name):
                attr = getattr(l, name)
                if attr is not None:
