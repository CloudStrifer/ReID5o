"""
Multi-Modal Retrieval Visualization Script for Tri-CUHK-PEDES
...
class MultiModalRetriever:
    ...
    def _build_model(self):
        # Load saved config to get correct num_classes
    ...
    def extract_gallery_features(self, gallery_loader) -> Tuple[torch.Tensor, torch.Tensor, List[str]]:
        """Extract features from gallery (RGB images)"""
        gids, gfeats, gpaths = [], [], []
    ...
    def extract_query_features(self,
        ...
        """Extract query features strictly matching metrics.py evaluation logic"""
...
def plot_multimodal_comparison(
    ...
    fig = plt.figure(figsize=(1.6 * (topk + 2), 3.5 * num_settings))
    gs = gridspec.GridSpec(num_settings, topk + 2, figure=fig, wspace=0.03, hspace=0.4)
    ...
    for row_idx, result in enumerate(modality_results):
        ...
        #             match_symbol = '✗'
        #
        #         ax_result.set_title(f"Rank {col_idx + 1} {match_symbol}\nID:{img_id+1}\n{score:.3f}",
        ...
        for col_idx in range(topk):
            ...
            try:
                ...
                # 2. 将画框的四条边都设置成对应的颜色，并加粗
                for spine in ax_result.spines.values():
                    spine.set_edgecolor(border_color)
                    spine.set_linewidth(4)  # 边框粗细，你可以根据需要改大或改小

                # 3. 原本的 ax_result.set_title(...) 已经被删掉，不再显示文字
    ...
    plt.suptitle(f"Tri-CUHK-PEDES Retrieval Comparison (Query ID: {query_id+1})",
...
def main():
    # TODO: 请确保将此路径修改为你在 CUHK-PEDES 上训练的模型 config 文件路径
    config_path = 'logs/configs.yaml'
    topk = 10

    print("=" * 80)
    print("Tri-CUHK-PEDES Multi-Modal Retrieval Visualization")
    print("=" * 80)
    ...
    # TODO: 修改此处为 Tri-CUHK-PEDES 测试集中的真实 ID、文本描述和草图路径
    query_data = {
        'query_id': 20,  #注意，查看打印的ID，如果查中的话，要根据这个图片显示的ID，减去1位，得到真实ID值
        'text': "She is wearing black heels, tight black leggings that go to the mid-calf, a black sleeveless top, and a large black hat with floppy brim. She has gold bracelets on her right wrist.",
        'sk_path': "data/sketch/aliyun/CUHK/imgs/test_query/p8856_s17672.jpg",
    }

    # 针对 Tri-CUHK-PEDES 仅对比这三种模态
    ...
    plot_multimodal_comparison(
        ...
        topk=topk,
        save_path='tri_cuhk_pedes_retrieval_comparison.png'
    )

    print("\n" + "=" * 80)
    print("✓ Tri-CUHK-PEDES retrieval visualization completed!")
    print("=" * 80)
